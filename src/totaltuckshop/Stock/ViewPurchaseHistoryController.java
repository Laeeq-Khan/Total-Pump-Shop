...
public class ViewPurchaseHistoryController implements Initializable {
    ...
    private void deletePurchaseItem(){
                ...
                int items = row.getItems();
                 String stockkk = Database_Returns.Code_Return("stock", "product_code", pCode, "items");
                int currentStock = Integer.parseInt(stockkk);
                if(currentStock<items){
                    Alert  a = new Alert(Alert.AlertType.WARNING);
                    a.setTitle("Message");
                    a.setContentText("You sold this product. Its not possible to return this item");
                    a.showAndWait();
                    return;
                }
                 detailsList.remove(index);
                String query = "update purchase_invoice_details set status='false' where sr='"+sr+"'";
    ...
    private void displayInvoiceDetail(String invoice){
        ...
        String query = "select * from purchase_invoice_details where status='true' and invoice_number='"+invoice+"'";
        try {
            Statement st = Connection_DB.db_Connection().createStatement();
    ...
    private void populating_invoices(String date) {
        ...
        String query = "select * from purchase_invoices where date='"+date+"'";
        try {
            Statement st = Connection_DB.db_Connection().createStatement();
            ResultSet rs = st.executeQuery(query);
            int count=0;
            ...
            while(rs.next()){
                String sr = rs.getString("sr");
                String invocie = rs.getString("invoice_number");
                String q  ="select * from purchase_invoice_details where invoice_number='"+invocie+"' and status='true'";
